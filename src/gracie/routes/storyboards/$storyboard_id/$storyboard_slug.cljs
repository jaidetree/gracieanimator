     ...
     (when (seq (:speakerdecks storyboard))
       [:div#boards.space-y-8 [:h2 "Boards"]
        (for [[idx deck] (map-indexed vector (get storyboard :speakerdecks []))]
          [:div.embedded-shots.relative.md:block {:key idx}
       ...
       [:div#pdfs.space-y-8 [:h2 "PDFs"]
        [:ul.bg-black.bg-opacity-20.p-8.md:block
         (for [pdf (get storyboard :pdfs [])]
           [:li {:key (:name pdf)}
            [:a.flex.flex-row
             {:href (download "downloads" (:url pdf)),
              :target (str "_grace_pdf_")} [:span.mr-2.text-lg [pdf-icon]]
             (:name pdf)]])]])
     (when (count blocks) [:div#content.space-y-8 (blocks->hiccup blocks)])
     #_[:pre (u/pprint-str (get storyboard :speakerdeck))]]
    [:aside.col-span-12.md:col-span-4.order-1.md:order-2
     [:div.space-y-4.sticky.top-8
      [:div.sidebar-section [:h2 "Category"]
       [:a {:href (str "/storyboards")} "Storyboards"]
       [:span.mx-4.text-white.text-opacity-50 "/"]
       [:a
        {:href (str "/storyboards/category/"
                    (u/slugify (:category storyboard)))}
        (:category storyboard)]]
      [:div.sidebar-section [:h2 "Navigation"]
      ...
      (when (or (pos? (count (get storyboard :pdfs [])))
        ...
        [:div.sidebar-section [:h2 "Quick Links"]
         [:ul.space-y-1
          (for [[idx url] (map-indexed vector
                                       (get storyboard :speakerdeck-urls []))]
            [:li {:key url}
             [:a.flex.flex-row.gap-2 {:href url} [:span [img-icon]]
              (str "Boards Set " (inc idx))]])
          (when (count (get storyboard :pdfs []))
            (for [pdf (get storyboard :pdfs [])]
              [:li {:key (:name pdf)}
               [:a.flex.flex-row.gap-2 {:href (download "downloads" (:url pdf))}
                [:span [pdf-icon]] (:name pdf)]]))]])]]]
