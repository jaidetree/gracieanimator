...
(defn view
  ...
  [:div
   [:div.grid.grid-cols-12.gap-8
    [:main.col-span-8.space-y-16
     [:div#animatic.space-y-8
      [:h1
       (:title storyboard)]
      [:div.embedded-video.relative
     ...
     (when (count (get storyboard :pdfs []))
       [:div#pdfs.space-y-8
        [:h2
         "PDFs"]
        [:ul.bg-black.bg-opacity-20.p-8
         (for [pdf (get storyboard :pdfs [])]
           [:li
            {:key (:name pdf)}
            [:a.flex.flex-row
             {:href (:url pdf)
              :target (str "_grace_pdf_" (:https://1aed85159e11.ngrok.io/storyboards/ODU5MTQwNA/valt-the-wonder-deer-storyboard-revisions-season-3#animatic))}
             [:span.mr-2.text-lg
              [pdf-icon]]
             (:name pdf)]])]])
